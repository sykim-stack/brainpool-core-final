...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.type === 'MANUAL_SNAPSHOT') {
    handleManualSnapshot(msg.data).then(sendResponse).catch(e => sendResponse({error: e.message}));
    return true;
  }
...
async function handleManualSnapshot(data) {
  console.log("[Snapshot] 데이터 수신:", data?.title);
  let ctx = { rawConversation: data };
  ctx = await SnapshotLayer(ctx);
  return { success: !ctx._error, error: ctx._error, summary: "테스트 요약" };
}
