...
async function handleHajunMessage(data = {}) {
  ...
  const payload = await hajunFetch('/api/hajun?action=post_message', {
    method: 'POST',
    body: JSON.stringify({
      yard_key,
      room_key,
  ...
  if (payload._error) return { success: false, error: payload._error };
  return { success: true, payload: payload.payload || null, summary: `${yard_key}/${room_key}에 Message 저장 완료` };
}
