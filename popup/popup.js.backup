...
async function generateContinuePrompt() {
  try {
    ...
    const text = await res.text();
    let contexts = [];
    try { contexts = JSON.parse(text); } catch(e) { throw new Error('응답 파싱 실패'); }
    if (contexts.length === 0) throw new Error('저장된 맥락 없음');

    const ctx = contexts[0];
    
    let rawSummary = ctx.summary || '';
    let nextAction = ctx.next_action || '';
    let lastTask = ctx.last_task || '';
    let prompt = '';
    
    if (rawSummary.length > 100 && !rawSummary.includes('분석중') && !rawSummary.includes('작업 진행 중')) {
      let truncatedSummary = rawSummary.length > 800 ? rawSummary.substring(0, 800) + '…(중략)' : rawSummary;
      prompt = `🦁 BRAINPOOL Core — 지난 대화 이야기를 이어갑니다.

📖 대화 내용 (요약)
${truncatedSummary}

🎯 마지막에 논의한 작업
${lastTask || '구체적인 작업을 논의 중'}

🚀 다음에 할 일
${nextAction || '맥락을 검토하고 자연스럽게 이어가기'}

---
위 내용은 당신과 사용자가 나눈 대화의 흐름입니다. 
사용자가 이어서 작업을 요청했습니다. 자연스럽게 이전 맥락을 유지하며 대화를 계속 진행해주세요.
(자동 실행 없음 / 복사 후 AI 채팅창에 붙여넣기)`;
    } else {
      prompt = `🦁 BRAINPOOL Core — 이전 대화를 이어갑니다.

📌 마지막 작업
${lastTask || '대화 기록 없음'}

📝 간단 요약
${rawSummary || '저장된 대화 내용이 없습니다.'}

🎯 다음 행동
${nextAction || '대화를 더 나눠보거나, 원하는 주제를 알려주세요.'}

---
위 내용을 참고하여 이전 맥락을 유지하며 작업을 이어서 진행해주세요.`;
    }
    
    await navigator.clipboard.writeText(prompt);
    alert('✅ 이야기 프롬프트가 복사되었습니다.\nClaude/ChatGPT에 붙여넣으면 맥락을 이어갑니다.');
  } catch(e) {
    alert(`❌ 실패: ${e.message}`);
  }
}
